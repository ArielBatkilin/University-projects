...
class GraphPlan(object):
    ...
    def graph_plan(self):
        ...
        while plan_solution is None:  # while we didn't extract a plan successfully
            ...
            pg_next.expand(self.graph[
                               level - 1])  # create next level of the graph by expanding
            ...
            if plan_solution is None and self.is_fixed(
                ...
                if len(self.no_goods[level - 1]) == len(self.no_goods[level]):
                    ...
                    return None
                size_no_good = len(self.no_goods[
                                       level])  # we didn't fail yet! update size of no good
    ...
    def gp_search(self, graph, sub_goals, _plan, level):
        ...
        prop = sub_goals[0]
        providers = []
        for action1 in [act for act in
                        graph[level].get_action_layer().get_actions() if
                        prop in act.get_add()]:
            no_mutex = True
    ...
    def is_fixed(self, level):
        ...
        if len(self.graph[level].get_proposition_layer().get_propositions()) == \
                len(self.graph[
                        level - 1].get_proposition_layer().get_propositions()) and \
                len(self.graph[
                        level].get_proposition_layer().get_mutex_props()) == \
                len(self.graph[
                        level - 1].get_proposition_layer().get_mutex_props()):
            return True
        return False
...
def independent_pair(a1, a2):
    ...
    """
    # check inconsistent
    a1_adds_list = a1.get_add()
    a2_adds_list = a2.get_add()
    # a1_del_list = a1.get_delete()
    # a2_del_list = a2.get_delete()
    for add in a1_adds_list:
        if a2.is_neg_effect(add):
            return False
    for add2 in a2_adds_list:
        if a1.is_neg_effect(add2):
            return False
    # if intersection(a1_adds_list, a2_del_list) or intersection(a1_del_list,
    #                                                            a2_adds_list):
    #     return False
    # check interference
    a1_pre_list = a1.get_pre()
    a2_pre_list = a2.get_pre()
    for pre in a1_pre_list:
        if a2.is_neg_effect(pre):
            return False
    for pre2 in a2_pre_list:
        if a1.is_neg_effect(pre2):
            return False
    # if intersection(a1_pre_list, a2_del_list) or intersection(a2_pre_list,
    #                                                           a1_del_list):
    #     return False
    return True
